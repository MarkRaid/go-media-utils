...
func cuttingWorker(jobs <-chan cutJob, done chan<- bool) {
	for job := range jobs {
		...
		for iterNum := 1; iterNum <= job.numFrames; iterNum++ {
			...
			cmd := exec.Command("ffmpeg", "-hide_banner", "-n", "-ss", strconv.FormatInt(int64(ssOffset), 10), "-i", job.filePath, "-map", "0:v:0", "-vf", "scale=iw*sar:ih", "-sws_flags", "sinc", "-frames:v", "1", "-q:v", "1", "-f", "image2", "-update", "1", absoluteOutputFileName)
